...
background_process = 'E240_nunuHX'
...
os.makedirs("pdf_results_nunuHX", exist_ok=True)
...
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('False Positive Rate (nunuHX Efficiency)')
plt.ylabel('True Positive Rate (qqHinvi Efficiency)')
plt.title('ROC Curve: qqHinvi vs nunuHX (Unweighted Training)')
plt.legend(loc="lower right")
plt.savefig("pdf_results_nunuHX/roc_curve.pdf")
plt.close()

# --- График важности признаков ---
xgb.plot_importance(model, max_num_features=14, importance_type='gain', height=0.8)
plt.title("Feature Importance (Gain) - qqHinvi vs nunuHX")
plt.tight_layout()
plt.savefig("pdf_results_nunuHX/feature_importance.pdf")
plt.close()
...
plt.hist(y_pred_proba[y_test == 0], bins=50, 
         alpha=0.6, color='blue', label='Background (nunuHX)', density=True)
plt.xlabel('BDT Score')
plt.ylabel('Density')
plt.title('BDT Output Distribution (Test Set)')
plt.legend()
plt.yscale('log')
plt.savefig("pdf_results_nunuHX/bdt_output_dist.pdf")
plt.close()

print("Графики сохранены в директорию pdf_results_nunuHX/")
